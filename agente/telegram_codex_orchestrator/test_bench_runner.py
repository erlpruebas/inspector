...
class TestBenchRunner:
    ...
    def run_all(self) -> list[dict[str, Any]]:
        ...
        for task in BENCH_TASKS:
            ...
            if not success and error_msg.startswith("WATCHDOG"):
                ...
                elif expected_file and not file_valid:
    ...
    def _validate_expected_file(self, task: dict[str, Any], file_path: Path) -> tuple[bool, str]:
        ...
        if task["id"] == 17:
            ...
            return ("800" in content), "suma.txt no contiene 800"
        if task["id"] == 19:
            content = file_path.read_text(encoding="utf-8", errors="replace")
            emails = re.findall(r"[\w.+-]+@[\w.-]+\.[A-Za-z]{2,}", content)
            return (len(set(emails)) >= 4), "correos.txt no contiene los 4 emails esperados"
        if task["id"] == 20:
            try:
                py_compile.compile(str(file_path), doraise=True)
            except py_compile.PyCompileError as exc:
                return False, f"programa_roto.py no compila: {exc.msg}"
            content = file_path.read_text(encoding="utf-8", errors="replace")
            return ("if calcular_cuadrado(5) > 10:" in content), "programa_roto.py no tiene el if corregido"
        if task["id"] == 21:
            content = file_path.read_text(encoding="utf-8", errors="replace")
            return ("FACT-1002" in content and "Ana" in content), "impagadas.txt no contiene la factura impagada esperada"
        if task["id"] == 25:
            content = file_path.read_text(encoding="utf-8", errors="replace")
            return ("8.75" in content and "7.75" in content), "reporte_promedios.csv no contiene promedios esperados"
        if task["id"] == 30:
            header = file_path.read_bytes()[:4]
            return (header == b"%PDF"), "reporte.pdf no es un PDF valido"
        return True, ""
