    ...
    *,
    ...
    try:
        ...
        if pause_after_paste:
            ...
            (run_dir / "result.json").write_text(json.dumps(result.to_dict(), indent=2, ensure_ascii=False), encoding="utf-8")
            return result
        screenshot_path = _wait_and_capture(run_dir, wait_seconds=wait_seconds)
        ...
        extracted_path = _extract_tokens_from_text(result_path.read_text(encoding="utf-8"), run_dir)
        result = DesktopCodexResult(
            ok=True,
...
def _input_coordinates(window, *, new_chat: bool) -> tuple[int, int]:
    calibrated = _calibrated_point("input_box")
    if calibrated:
        return calibrated
    profiled = _profile_point("initial_input" if new_chat else "browser_input")
    if profiled:
        return profiled
...
def _click_send_button(window, *, new_chat: bool) -> bool:
    calibrated = _calibrated_point("send_button")
    if calibrated:
        _click_point(calibrated)
        return True
    profiled = _profile_point("initial_send" if new_chat else "browser_send_stop")
    if profiled:
        _click_point(profiled)
        return True
    return False
...
def _wait_and_capture(run_dir: Path, *, wait_seconds: int) -> Path:
...
def main() -> int:
    ...
    parser.add_argument("--resume-send", action="store_true", help="Solo pulsa enviar sobre una conversación ya preparada.")
